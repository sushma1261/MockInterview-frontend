...
/**
 * Shared chat interface component used by both:
 * - /chat (new interviews with optional resume)
 * - /chat/[sessionId] (resumed sessions)
 */
export default function ChatInterface({
  ...
  const {
    ...
    skipQuestion,
    restartInterview,
  } = useInterviewChat({ resumeId, sessionId });
  ...
  // Show resume upload screen if interview hasn't started and upload is enabled
  if (!interviewStarted && showResumeUpload) {
    return (
      <ResumeUploadSection
            ...
            {showSessionBanner && (
              <div className="mb-4 p-4 bg-indigo-900/30 border border-indigo-700 rounded-lg">
                <p className="text-indigo-300 text-sm text-center">
                  📝 Session resumed - Continue from where you left off
                </p>
              </div>
            )}
